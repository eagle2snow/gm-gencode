...
public class ControllerFactory implements IBuildCodeFactory {
	...
	public void buildCode() throws IOException, TemplateException {
		...
		String path = entityPackage.replace("base", "admin").replaceAll("com.gm.", "").replaceAll("model.", "")
		...
		String workspace = System.getProperty("user.dir").replaceAll("aj-base", "");

		// workspace = workspace.substring(0, workspace.length()-1);
		String projectJavaDir = workspace + File.separator + "aj-main" + File.separator + "src" + File.separator
		...
		File tplFile = null;
		projectJavaDir = projectJavaDir.replace("aj-main", "aj-gencode");
