...
public class IDaoFactory implements IBuildCodeFactory {
	...
	private String tplName = "IDao.ftl";
	...
	public static IDaoFactory buildIDaoFactory(Class<?> cl) {
		IDaoFactory idaoFactory = new IDaoFactory();
		idaoFactory.cl = cl;
		return idaoFactory;
	}
	...
	public void buildCode() throws IOException, TemplateException {
		...
		if (this.codeOutPath == null) {
			outPath = (entityPackage.replaceAll("\\.", "\\\\")).replaceAll("model", "dao").replaceAll(entitySimpleName,"");
		...
		projectJavaDir = projectJavaDir.replace("aj-base", "aj-gencode");
		...
		String fileName = outPath + "I" + entitySimpleName + "Dao.java";
