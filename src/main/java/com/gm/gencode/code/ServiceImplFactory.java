...
public class ServiceImplFactory implements IBuildCodeFactory {
	...
	private String tplName = "ServiceImpl.ftl";

	public static ServiceImplFactory buildServiceImplFactory(Class<?> cl) {
		ServiceImplFactory factory = new ServiceImplFactory();
		factory.setCl(cl);
		return factory;
	}
	...
	public void buildCode() throws IOException, TemplateException {
		...
		String _package = entityPackage.replaceAll("model", "service").replaceAll(entitySimpleName, "impl");
		String idaoPackage = _package.replaceAll("impl", "I" + entitySimpleName + "Dao").replaceAll("service", "dao");

		String iservicePackage = _package.replaceAll("impl", "I" + entitySimpleName + "Service");
		map.put("entityName", StringUtil.firstCase(entitySimpleName));
		map.put("iservicPackage", iservicePackage.replace(".base", ""));
		map.put("package", _package.replace(".base", ""));
		...
		if (this.tplPath == null) {
			tplFile = new File(projectJavaDir.replace("aj-base", "aj-gencode") + "com\\gm\\gencode\\tpl\\");
		} else {
			tplFile = new File(projectJavaDir.replace("aj-base", "aj-gencode") + this.tplPath);
		}
		...
		outPath = outPath.replaceAll("\\\\aj-base\\\\", "\\\\aj-service\\\\");
		outPath = outPath.replace("\\base", "");
		File file = new File(outPath);
		IOStreamUtil.mkDir(file);

		String fileName = outPath + entitySimpleName + "ServiceImpl.java";
